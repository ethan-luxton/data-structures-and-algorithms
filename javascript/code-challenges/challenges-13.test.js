...
const longestString = (arr) => {
  return arr.length > 0 ? arr.indexOf(arr.reduce((a, b) => a.length > b.length ? a:b)) : -1 
};
...
const findHappiness = (arr) => {
  return arr.filter(e => e ? e.includes(':)') : arr.pop(e));
};
...
const onlyOddChars = (str) => {
  return str.split('').filter((c, i) => i % 2 !== 0).join('')
};
