...
const updateNumbers = (obj) => {
  ...
  return Object.keys(obj).map((key, idx) => `${key}` + ':' + ` ${objArr[idx]}`);
};
...
const getHouses = (arr) => {
  return arr.map((object) => object.house)
};
...
const hasChildrenValues = (arr, character) => {
  for (let obj of arr) {
    if (obj.name === character && obj.children) {
      return true;
    } else if (obj.name === character && !obj.children) {
      return false;
    }
  }
  // Literally could not find a way to implement Object.values()
};
