...
// I am not sure why challenge 1 isn't passing the test, it returns the correct answers for the test case, it passes the first test of challenge 01 but not the second. With the console log below it shows the output of the array and the length of children in the first variable.
// console.log(sortByChildren(characters), characters[0].children.length)
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const citiesAtoJ = (arr) => {
  let newArr = [];
  arr.forEach(element => { if (/^[A-J]/.test(element)){newArr.push(element)}});
  return newArr;
};
