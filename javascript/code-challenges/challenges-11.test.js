...
function transformToLis(obj){
  ...
  function checkValue () {
    if (key.length == 0) {
      ...
      return [`<li>${key[0]}: ${value[0]}</li>`, `<li>${key[1]}: ${value[1]}</li>`]
    }
  }
   return checkValue()
};
...
const count = (target, input) => {
  ...
  for (let element of input) {
    for (let elementTwo of element) {
      modArr.push(elementTwo)
    }
  }
  return modArr.filter(num => num === target).length
};
...
const totalSum = (input) => {
  ...
  return modArr.reduce((prevNum, curNum) => prevNum + curNum)
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let element of input) {
    let ghostArr = [];
    element.forEach(int => Number.isInteger(int) ? (int % 5 === 0 ? ghostArr.push(Math.pow(2, int)) : 0): 0);
    newArr.push(ghostArr);
  }
  return newArr;
};
...
let findMaleAndFemale = (data) => {
  let str = ''
  data.forEach(e => e.gender === 'male' || e.gender === 'female' ? (str == '' ? str = e.name : str += ` and ${e.name}`) : 0);
  return str
};
...
let findShortest = (data) => {
  let newArr = [];
  let ans;
  data.forEach(e => {newArr.push(Number(e.height));})
  let hit = Math.min(...newArr)
  data.forEach(e => {if (hit == e.height){ans = e.name}})
  return ans
};
