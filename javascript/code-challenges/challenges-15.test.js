...
const screenForNames = (arr) => {
  let newArr = [];
  arr.forEach(s => s.search(/^(Dr\.|Mr\.|Mrs\.|Ms\.)\s[A-Za-z][A-Za-z\s]+$/) === -1 ? 0 : newArr.push(s));
  return newArr;
}
...
let biggerThanLuke = (arr) => {
  let str = ''
  arr.forEach(e => e.mass > 77 ? (str === '' ? str = e.name : str += ` - ${e.name}`) : 0);
  return str
};
...
------------------------------------------------------------------------------------------------ */
const check = (r1, c1, r2, c2, r3, c3, board) => {
...
const detectTicTacToeWin = (board) => {
  ...
  for(let i = 0; i < board.length; i++){
    check(i, 0, i, 1, i, 2, board) || check(0, i, 1, i, 2, i, board) ? bool = true : 0;
  }
  check(0, 0, 1, 1, 2, 2, board) || check(0, 2, 2, 2, 2, 0, board) ? bool = true : 0;
  return bool;
};
